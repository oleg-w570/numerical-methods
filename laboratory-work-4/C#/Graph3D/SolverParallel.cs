...
namespace Graph3D
{
    internal class SolverParallel
    {
        ...
        public SolverParallel(in TestTask3 task, int n = 8, int m = 8, double eps = 0.001, int maxiter = 1000,
            int K = 2)
        {
            this.task = task;
            this.n = n;
        ...
        private double CalculateDiscrepancy()
        {
            ...
            for (var j = 1; j < borderBottom; j++)
                for (var i = 1; i < n; i++)
                {
                    var R = Math.Abs(A * u[i, j] + h2 * (u[i - 1, j] + u[i + 1, j]) +
                        k2 * (u[i, j - 1] + u[i, j + 1]) - task.F(x[i], y[j]));
                    resR += R * R;
                }
            ...
            for (var j = borderBottom; j < borderTop + 1; j++)
            {
                ...
                {
                    var R = Math.Abs(A * v[i, j] + h2 * (v[i - 1, j] + v[i + 1, j]) +
        ...
        private double[,] ChebishevLocalIteration(double[,] vcurr, int s)
        {
            var t = tau[s];
            ...
            Parallel.For(borderBottom, borderTop + 1, j =>
            {
                Parallel.For(1, borderInsideLeft, i =>
                {
                    vnew[i, j] = vcurr[i, j] * (1 + t * A)
                                 + t * (k2 * (vcurr[i, j - 1] + vcurr[i, j + 1]) +
                                     h2 * (vcurr[i - 1, j] + vcurr[i + 1, j]) - task.F(x[i], y[j]));
                });
                Parallel.For(borderInsideRight + 1, borderRight, i =>
                {
                    vnew[i, j] = vcurr[i, j] * (1 + t * A)
                                 + t * (k2 * (vcurr[i, j - 1] + vcurr[i, j + 1]) +
                                     h2 * (vcurr[i - 1, j] + vcurr[i + 1, j]) - task.F(x[i], y[j]));
        ...
        private double VectorDiffNorm(double[,] v1, double[,] v2)
        {
            double maxdiff = 0;
