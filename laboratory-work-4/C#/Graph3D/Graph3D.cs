...
namespace Plot3D
{
    ...
    public class Graph3D : UserControl
    {
        ...
        // The default size of the scatter objects (radius for circles)
        ...
        private static readonly double[] VALUES_RHO = { 300, 1800, 1350, 2 };
        private static readonly double[] VALUES_THETA = { 10, 170, 70, 0.25 }; // degree
        private static readonly double[] VALUES_PHI = { 0, 360, 230, 0.4 }; // degree  (continuous rotation)

        private eRaster me_Raster = eRaster.Off;
        private readonly SolidBrush[] mi_AxisBrushes = new SolidBrush[3];
        private readonly Pen[] mi_AxisPens = new Pen[3];
        private Pen mi_BorderPen;
        private readonly List<cDrawObj> mi_DrawObjects = new List<cDrawObj>(); // cPolygon or cLine or cScatter
        private cMinMax3D mi_MinMax;
        private readonly cMouse mi_Mouse = new cMouse();
        private cPoint3D[,] mi_PolyArr;
        private Pen mi_PolyLinePen;
        private cQuadrant mi_Quadrant;
        private readonly Pen[] mi_RasterPens = new Pen[3];
        private cScatter[] mi_ScatterArr;
        private SolidBrush[] mi_SchemeBrushes;
        private Pen[] mi_SchemePens;
        private SolidBrush mi_TopLegendBrush;
        private readonly cTransform mi_Transform = new cTransform();
        private Point mk_Offset2D;
        private readonly string[] ms_AxisLegends = new string[3];
        ...
        public string AxisX_Legend
        {
            set
            {
                ms_AxisLegends[(int)eCoord.X] = value;
                Invalidate();
            }
            get => ms_AxisLegends[(int)eCoord.X];
        }
        ...
        public string AxisY_Legend
        {
            set
            {
                ms_AxisLegends[(int)eCoord.Y] = value;
                Invalidate();
            }
            get => ms_AxisLegends[(int)eCoord.Y];
        }
        ...
        public string AxisZ_Legend
        {
            set
            {
                ms_AxisLegends[(int)eCoord.Z] = value;
                Invalidate();
            }
            get => ms_AxisLegends[(int)eCoord.Z];
        }
        ...
        public Color AxisX_Color
        {
            set
            {
                SetAxisColor(eCoord.X, value);
                Invalidate();
            }
            get => mi_AxisPens[(int)eCoord.X].Color;
        }
        ...
        public Color AxisY_Color
        {
            set
            {
                SetAxisColor(eCoord.Y, value);
                Invalidate();
            }
            get => mi_AxisPens[(int)eCoord.Y].Color;
        }
        ...
        public Color AxisZ_Color
        {
            set
            {
                SetAxisColor(eCoord.Z, value);
                Invalidate();
            }
            get => mi_AxisPens[(int)eCoord.Z].Color;
        }
        ...
        public void SetColorScheme(Color[] c_Colors, float f_LineWidth)
        {
            ...
            for (var i = 0; i < mi_SchemeBrushes.Length; i++)
            {
        ...
        {
            Debug.Assert(!InvokeRequired); // Do not call from other threads or use Invoke()
            mi_Mouse.AssignTrackbar(eMouseAction.Rho, i_Rho, OnTrackbarScroll);
            mi_Mouse.AssignTrackbar(eMouseAction.Theta, i_Theta, OnTrackbarScroll);
            mi_Mouse.AssignTrackbar(eMouseAction.Phi, i_Phi, OnTrackbarScroll);
        }
        ...
        /// </summary>
        public void SetFunction(delRendererFunction f_Function, PointF k_Start, PointF k_End, double d_Density,
            eNormalize e_Normalize)
        {
            Debug.Assert(!InvokeRequired); // Do not call from other threads or use Invoke()

            var s32_CountX = (int)((k_End.X - k_Start.X) / d_Density + 1);
            var s32_CountY = (int)((k_End.Y - k_Start.Y) / d_Density + 1);

            var i_Points3D = new cPoint3D[s32_CountX, s32_CountY];

            for (var C = 0; C < s32_CountX; C++)
            {
                var X = k_Start.X + d_Density * C;

                for (var R = 0; R < s32_CountY; R++)
                {
                    var Y = k_Start.Y + d_Density * R;
                    var Z = f_Function(X, Y);
        ...
        public void SetScatterLines(cScatter[] i_ScatterArr, eNormalize e_Normalize, float f_LineWidth)
        {
            ...
            foreach (var i_Scatter in i_ScatterArr)
            {
        ...
        {
            if (mi_MinMax.md_MaxX == mi_MinMax.md_MinX)
            {
                mi_MinMax.md_MinX -= 1.0;
                mi_MinMax.md_MaxX += 1.0;
            }

            if (mi_MinMax.md_MaxY == mi_MinMax.md_MinY)
            {
                mi_MinMax.md_MinY -= 1.0;
                mi_MinMax.md_MaxY += 1.0;
            }

            if (mi_MinMax.md_MaxZ == mi_MinMax.md_MinZ)
            {
                mi_MinMax.md_MinZ -= 1.0;
                mi_MinMax.md_MaxZ += 1.0;
            }

            // Normalize 3D X,Y values to compensate different ranges of X min ... X max and Y min ... Y max
            var d_RangeX = mi_MinMax.md_MaxX - mi_MinMax.md_MinX;
            var d_RangeY = mi_MinMax.md_MaxY - mi_MinMax.md_MinY;
            ...
            switch (e_Normalize)
            {
                case eNormalize.MaintainXY:
                    var d_RangeXY = (d_RangeX + d_RangeY) / 2; // average
                    d_RangeX = d_RangeXY;
                    d_RangeY = d_RangeXY;
                    break;

                case eNormalize.MaintainXYZ:
                    var d_RangeXYZ = (d_RangeX + d_RangeY + d_RangeZ) / 3; // average
                    d_RangeX = d_RangeXYZ;
                    d_RangeY = d_RangeXYZ;
                    d_RangeZ = d_RangeXYZ;
                    break;
        ...
        private void CreateCoordinateSystem(Graphics i_Graph)
        {
            ...
            var i_Lines = new List<cLine>();

            // Add the 3 coordinate system main axis
            for (var A = 0; A < 3; A++)
            {
                var i_Axis = new cLine();
                ...
                switch ((eCoord)A)
                {
                    case eCoord.X: // Blue
                        ...
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_Y =
                            Math.Min(0.0, mi_MinMax.md_MinY * AXIS_EXCESS); // X axis at minimum Y position
                        i_Axis.mi_Points3D[1].md_Y =
                            Math.Min(0.0, mi_MinMax.md_MinY * AXIS_EXCESS); // X axis at minimum Y position
                    ...
                    case eCoord.Y: // Green
                        ...
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_X =
                            Math.Min(0.0, mi_MinMax.md_MinX * AXIS_EXCESS); // Y axis at minimum X position
                        i_Axis.mi_Points3D[1].md_X =
                            Math.Min(0.0, mi_MinMax.md_MinX * AXIS_EXCESS); // Y axis at minimum X position
                    ...
                    case eCoord.Z: // Red
                        ...
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_X =
                            Math.Min(0.0, mi_MinMax.md_MinX * AXIS_EXCESS); // Z axis start at minimum X position
                        i_Axis.mi_Points3D[1].md_X =
                            Math.Min(0.0, mi_MinMax.md_MinX * AXIS_EXCESS); // Z axis start at minimum X position
                        i_Axis.mi_Points3D[0].md_Y =
                            Math.Min(0.0, mi_MinMax.md_MinY * AXIS_EXCESS); // Z axis start at minimum Y position
                        i_Axis.mi_Points3D[1].md_Y =
                            Math.Min(0.0, mi_MinMax.md_MinY * AXIS_EXCESS); // Z axis start at minimum Y position
            ...
            mi_Quadrant = new cQuadrant(mi_Mouse.md_Phi, i_Lines[(int)eCoord.X], i_Lines[(int)eCoord.Y],
                i_Lines[(int)eCoord.Z]);

            // Add raster lines in 6 different directions
            if (me_Raster >= eRaster.Raster)
                for (var A = 0; A < 3; A++) // iterate axis X,Y,Z
                {
                    // Combine X+Y, Y+Z, Z+X
                    var e_First = (eCoord)A;
                    var e_Second = (eCoord)((A + 1) % 3);

                    for (var D = 0; D < 2; D++) // iterate first, second
                    {
                        var i_FirstAxis = i_Lines[(int)e_First];
                        var i_SecondAxis = i_Lines[(int)e_Second];

                        var d_SecndStart = i_SecondAxis.mi_Points3D[0].GetValue(e_Second);
                        var d_SecndEnd = i_SecondAxis.mi_Points3D[1].GetValue(e_Second);

                        // Distance between raster lines
                        var d_Interval = CalculateInterval(d_SecndEnd - d_SecndStart);

                        for (var L = -11; L < 11; L++) // iterate raster line
                        {
                            var d_Offset = d_Interval * L;

                            if (d_Offset < d_SecndStart || d_Offset > d_SecndEnd)
                                continue;

                            var i_Raster = new cLine();
                            ...
                            else // X + Y Red
                            {
                                ...
                                var i_AxisZ = i_Lines[(int)eCoord.Z];
            ...
            {
                var s32_LabelWidth = 0;
                foreach (var i_Line in i_Lines)
                    if (i_Line.me_Line == eCoord.Y && i_Line.me_Offset == eCoord.Z)
                    {
                        var s_Label = FormatLabel(i_Line.md_Label);
                        var k_Size = i_Graph.MeasureString(s_Label, Font);
        ...
        {
            var i_Points2D = new cPoint2D[mi_PolyArr.GetLength(0), mi_PolyArr.GetLength(1)];

            // Convert 3D --> 2D
            for (var X = 0; X < mi_PolyArr.GetLength(0); X++)
            for (var Y = 0; Y < mi_PolyArr.GetLength(1); Y++)
                i_Points2D[X, Y] = mi_Transform.Project(mi_PolyArr[X, Y], mi_MinMax.mi_Center3D);

            // Create polygons
            for (var X = 0; X < mi_PolyArr.GetLength(0) - 1; X++)
            for (var Y = 0; Y < mi_PolyArr.GetLength(1) - 1; Y++)
            {
                var i_Poly = new cPolygon(i_Points2D[X, Y],
                    i_Points2D[X, Y + 1],
                    i_Points2D[X + 1, Y + 1],
                    i_Points2D[X + 1, Y]);

                var Z1 = mi_PolyArr[X, Y].md_Z;
                var Z2 = mi_PolyArr[X, Y + 1].md_Z;
                var Z3 = mi_PolyArr[X + 1, Y + 1].md_Z;
                var Z4 = mi_PolyArr[X + 1, Y].md_Z;
                var Zavrg = (Z1 + Z2 + Z3 + Z4) / 4.0;
                ...
                var d_Sort = mi_Transform.ProjectXY(X + 1, Y + 1); // +1 because Z axis is at 0,0
        ...
        {
            // Convert 3D --> 2D
            foreach (var i_Scatter in mi_ScatterArr)
            {
                ...
                if (i_Scatter.mi_Brush == null)
                    i_Scatter.md_FactorZ = (i_Scatter.mi_Point3D.md_Z - mi_MinMax.md_MinZ) /
                                           (mi_MinMax.md_MaxZ - mi_MinMax.md_MinZ);

                // Scatter circles must be painted in correct order: from back to front. Order depends on rotation angle.
                var d_Sort = mi_Transform.ProjectXY(i_Scatter.mi_Point3D.md_X + 1.0,
        ...
        private Brush GetSchemeBrush(double d_FactorZ)
        {
            ...
            var s32_Index = (int)(d_FactorZ * (mi_SchemeBrushes.Length - 1));
        ...
        private Pen GetSchemePen(double d_FactorZ)
        {
            ...
            // d_FactorZ is a value between 0.0 and 1.0
            var s32_Index = (int)(d_FactorZ * (mi_SchemePens.Length - 1));
        ...
        {
            var s32_Red = c_Color.R + (255 - c_Color.R) / 2;
            var s32_Green = c_Color.G + (255 - c_Color.G) / 2;
            var s32_Blue = c_Color.B + (255 - c_Color.B) / 2;
        ...
        {
            var d_Factor = Math.Pow(10.0, Math.Floor(Math.Log10(d_Range)));
            if (d_Range / d_Factor >= 5.0)
                return d_Factor;
            if (d_Range / (d_Factor / 2.0) >= 5.0)
                return d_Factor / 2.0;
            return d_Factor / 5.0;
        }
        ...
        {
            var i_Bmp = new Bitmap(ClientSize.Width, ClientSize.Height);
            using (var i_Graph = Graphics.FromImage(i_Bmp))
            {
                Draw(i_Graph);
            }
        ...
        private void Draw(Graphics i_Graph)
        {
            ...
            var X = 4;
            var Y = ClientSize.Height - Font.Height - 4;
            for (var i = 2; i >= 0; i--)
            {
                if (string.IsNullOrEmpty(ms_AxisLegends[i]))
                    continue;

                var s_Disp = string.Format("{0}: {1}", (eCoord)i, ms_AxisLegends[i]);
            ...
            {
                string[] s_Legend = { "Rotation:", "Elevation:", "Distance:" };
                string[] s_Value =
                {
                    string.Format("{0:+#;-#;0}°", (int)mi_Mouse.md_Phi),
                    string.Format("{0:+#;-#;0}°", (int)mi_Mouse.md_Theta),
                    string.Format("{0}", (int)mi_Mouse.md_Rho)
                };

                var k_Size = i_Graph.MeasureString(s_Legend[1], Font); // measure the widest string
                X = 4;
                Y = 3;
                for (var i = 0; i < 3; i++)
                {
            ...
            var e_Smooth = SmoothingMode.Invalid;

            foreach (var i_DrawObj in mi_DrawObjects)
            {
                ...
                if (i_DrawObj.mi_Polygon != null)
                {
                    ...
                    var i_Poly = i_DrawObj.mi_Polygon;
                    var i_Brush = GetSchemeBrush(i_Poly.md_FactorZ);
                    i_Graph.FillPolygon(i_Brush, i_Poly.mk_Points);

                    if (mi_PolyLinePen != null) i_Graph.DrawPolygon(mi_PolyLinePen, i_Poly.mk_Points);
                }
                else if (i_DrawObj.mi_Scatter != null)
                {
                    ...
                    {
                        var i_Brush = i_Scatter.mi_Brush;
                        if (i_Brush == null) // user has not defined a color
                            i_Brush = GetSchemeBrush(i_Scatter.md_FactorZ);

                        switch (i_Scatter.me_Shape)
                        {
                            case eScatterShape.Circle:
                                i_Graph.FillEllipse(i_Brush, i_Scatter.mk_Point.X, i_Scatter.mk_Point.Y,
                                    i_Scatter.ms32_Diameter, i_Scatter.ms32_Diameter);
                                break;

                            case eScatterShape.Square:
                                i_Graph.FillRectangle(i_Brush, i_Scatter.mk_Point.X, i_Scatter.mk_Point.Y,
                                    i_Scatter.ms32_Diameter, i_Scatter.ms32_Diameter);
                                break;

                            case eScatterShape.Triangle:
                                i_Graph.FillPolygon(i_Brush, i_Scatter.mk_Polygon);
                                break;
                        }
                    }
                }
                else // cLine
                {
                    ...
                    {
                        var k_Pos = i_Line.mi_Points2D[1].Coord;
                        var i_Align = new StringFormat();
                        if (i_Line.me_Line == eCoord.Y && i_Line.me_Offset == eCoord.Z)
                        {
                            k_Pos.X += 5;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            var s32_DiffX = e.X - mi_Mouse.mk_LastPos.X;
            var s32_DiffY = e.Y - mi_Mouse.mk_LastPos.Y;
            mi_Mouse.mk_LastPos = e.Location;
        ...
        public class cPoint2D
        {
            ...
            public PointF Coord => new PointF((float)md_X, (float)md_Y);

            public bool IsValid =>
                // The screen will always be smaller than 9999 pixels
                !double.IsNaN(md_X) && Math.Abs(md_X) < 9999.9 &&
        ...
        private class cMinMax3D
        {
            ...
            public readonly cPoint3D mi_Center3D = new cPoint3D();

            // Constructor
            public cMinMax3D(cPoint3D[,] i_Points3D)
            {
                for (var X = 0; X < i_Points3D.GetLength(0); X++)
                for (var Y = 0; Y < i_Points3D.GetLength(1); Y++)
                {
                    var i_Point3D = i_Points3D[X, Y];
            ...
            public cMinMax3D(cScatter[] i_ScatterArr)
            {
                foreach (var i_Scatter in i_ScatterArr)
                {
                    var i_Point3D = i_Scatter.mi_Point3D;
        ...
        private class cLine
        {
            ...
            public Pen mi_Pen;
            public readonly cPoint2D[] mi_Points2D = new cPoint2D[2] { new cPoint2D(), new cPoint2D() };

            public readonly cPoint3D[]
                mi_Points3D = new cPoint3D[2] { new cPoint3D(), new cPoint3D() }; // start and end point of line

            // Graphics.DrawLine() with huge length may take several seconds and block the GUI thread --> hanging.
            public bool IsValid => mi_Points2D[0].IsValid && mi_Points2D[1].IsValid;
            ...
            {
                var d_DX = mi_Points2D[1].md_X - mi_Points2D[0].md_X;
                var d_DY = mi_Points2D[1].md_Y - mi_Points2D[0].md_Y;
        ...
        private class cPolygon
        {
            ...
            public readonly PointF[] mk_Points; // 4 polygon points
            ...
            public cPolygon(params cPoint2D[] i_Points2D)
            {
                ...
                for (var i = 0; i < i_Points2D.Length; i++)
        ...
        public class cScatter
        {
            ...
            public cScatter(double X, double Y, double Z, Brush i_Brush,
                int s32_Radius = SCATTER_SIZE,
                eScatterShape e_Shape = eScatterShape.Circle)
            {
                if (s32_Radius < 1) throw new ArgumentException();

                mi_Point3D = new cPoint3D(X, Y, Z);
                mi_Brush = i_Brush;
                ms32_Diameter = s32_Radius * 2;
                me_Shape = e_Shape;
            }
        ...
        {
            public readonly double md_Sort; // sorting is important. Always draw from back to front.
            public readonly cLine mi_Line;
            public readonly cPolygon mi_Polygon;
            public readonly cScatter mi_Scatter;

            public cDrawObj(cPolygon i_Polygon, double d_Sort)
            {
                mi_Polygon = i_Polygon;
                IsValid = i_Polygon.IsValid;
                md_Sort = d_Sort;
            }
        ...
        {
            public readonly bool mb_BottomView;
            public readonly double md_SortXY; // Sort order of raster in area XY  (red)
            public readonly double md_SortXZ; // Sort order of X axis and raster in area XZ (blue)
            public readonly double md_SortYZ; // Sort order of Y axis and raster in area YZ (green)
            public readonly int ms32_Quadrant;
            ...
            public cQuadrant(double d_Phi, cLine i_AxisX, cLine i_AxisY, cLine i_AxisZ)
            {
                ...
                var s32_Section45 = (int)d_Phi + 45;
                ...
                {
                    case 0:
                        mb_BottomView = i_AxisX.md_Angle < 180.0;
                        break;
                    case 1:
                        mb_BottomView = i_AxisY.md_Angle < 180.0;
                        break;
                    case 2:
                        mb_BottomView = i_AxisX.md_Angle > 180.0;
                        break;
                ...
                if (mb_BottomView)
                    ...
                    {
                        case 0:
                            ms32_Quadrant = i_AxisX.md_Angle + 180.0 < i_AxisZ.md_Angle ? 1 : 0;
                            break;
                        case 1:
                            ms32_Quadrant = i_AxisY.md_Angle + 180.0 < i_AxisZ.md_Angle ? 2 : 1;
                            break;
                        case 2:
                            ms32_Quadrant = i_AxisX.md_Angle < i_AxisZ.md_Angle ? 3 : 2;
                            break;
                        case 3:
                            ms32_Quadrant = i_AxisY.md_Angle < i_AxisZ.md_Angle ? 0 : 3;
                            break;
                    }
                else // Top View
                    switch (s32_Section45)
                    {
                        case 0:
                            ms32_Quadrant = i_AxisX.md_Angle > i_AxisZ.md_Angle ? 1 : 0;
                            break;
                        case 1:
                            ms32_Quadrant = i_AxisY.md_Angle > i_AxisZ.md_Angle ? 2 : 1;
                            break;
                        case 2:
                            ms32_Quadrant = i_AxisX.md_Angle + 180.0 > i_AxisZ.md_Angle ? 3 : 2;
                            break;
                        case 3:
                            ms32_Quadrant = i_AxisY.md_Angle + 180.0 > i_AxisZ.md_Angle ? 0 : 3;
                            break;
                    }
        ...
        private class cTransform
        {
            ...
            public void SetCoeficients(cMouse i_Mouse)
            {
                ...
                var d_Theta = i_Mouse.md_Theta * Math.PI / 180.0; // Height   of viewer (elevation)
                var d_Phi = (i_Mouse.md_Phi - 180.0) * Math.PI / 180.0; // Rotation around center (-pi ... +pi)
            ...
            {
                var d_Width = k_Size.Width * 0.0254 / 96.0; // 0.0254 m = 1 inch. Screen has 96 DPI
                var d_Height = k_Size.Height * 0.0254 / 96.0;
            ...
            {
                var X = (i_Point3D.md_X - i_Center3D.md_X) * md_NormalizeX;
                var Y = (i_Point3D.md_Y - i_Center3D.md_Y) * md_NormalizeY;
                var Z = (i_Point3D.md_Z - i_Center3D.md_Z) * md_NormalizeZ;
                ...
                var xn = -md_sf * X + md_cf * Y;
                var yn = -md_cf * md_ct * X - md_sf * md_ct * Y + md_st * Z;
                var zn = -md_cf * md_st * X - md_sf * md_st * Y - md_ct * Z + md_Rho;

                if (zn <= 0) zn = 0.01;

                // Thales' theorem
                var i_Point2D = new cPoint2D();
